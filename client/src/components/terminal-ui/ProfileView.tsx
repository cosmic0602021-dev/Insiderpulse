...
const ProfileView: React.FC<ProfileViewProps> = ({ lang, onRedeemCoupon }) => {
  ...
  const verifyTossAuthStatus = async () => {
    try {
      ...
      console.log('[ProfileView] ENV_CONFIG.isAppintos:', ENV_CONFIG.isAppintos);
      console.log('[ProfileView] window.location.href:', window.location.href);
      ...
      if (tossUser?.id) {
        ...
        console.log('[ProfileView] ❌ User not authenticated - showing login button');
        setTossUserId(null);
        // 기존에 잘못 저장된 데이터 정리
        localStorage.removeItem('appintos_user_id');
        alert('❌ 로그인 세션 없음 - 로그인 버튼 표시');
      }
    } catch (error) {
  ...
  return (
    <div className="flex-1 flex flex-col h-full overflow-hidden bg-[#050505]">
        ...
        <div className="flex-1 overflow-y-auto p-6 max-w-4xl mx-auto w-full space-y-6 custom-scrollbar">
            {/* Account Info - Hide in Apps-in-Toss (Toss login doesn't provide email) */}
            ...
            {/* Apps-in-Toss: User Info & Free Service */}
            {ENV_CONFIG.isAppintos && (
                <>
                    {/* 사용자 정보 섹션 */}
                    <div className="bg-[#0a0a0a] border border-neutral-900 p-6 rounded-sm">
                    ...
                    <div className="bg-[#0a0a0a] border border-neutral-900 p-6 rounded-sm">
                        <div className="flex items-center gap-3 mb-6 border-b border-neutral-800 pb-4">
                            ...
                            <h2 className="text-lg font-bold text-neutral-300">무료 서비스</h2>
                        </div>

                        <div className="bg-gradient-to-r from-emerald-900/30 to-blue-900/30 border border-emerald-900/50 p-4 flex justify-between items-center mb-6">
                            <div>
                            ...
                            <span className="px-3 py-1 text-xs font-bold uppercase tracking-wider border rounded-full bg-emerald-900/30 text-emerald-500 border-emerald-900/50">
                                무료 이용
                            </span>
                        </div>
                        ...
                        <div className="space-y-4 mb-6">
                            <p className="text-sm text-neutral-400">
                                앱인토스에서는 모든 프리미엄 기능을 무료로 이용할 수 있습니다.
                            </p>
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                                <div className="flex items-center gap-3 p-3 bg-neutral-900/50 border border-neutral-800 rounded">
                                    <Zap className="text-emerald-500" size={16} />
                                    <span className="text-xs text-neutral-300">실시간 내부자 거래 데이터</span>
                                </div>
                                <div className="flex items-center gap-3 p-3 bg-neutral-900/50 border border-neutral-800 rounded">
                                    <BarChart3 className="text-emerald-500" size={16} />
                                    <span className="text-xs text-neutral-300">AI 분석 및 신호 추적</span>
                                </div>
                                <div className="flex items-center gap-3 p-3 bg-neutral-900/50 border border-neutral-800 rounded">
                                    <Crown className="text-emerald-500" size={16} />
                                    <span className="text-xs text-neutral-300">모든 프리미엄 기능</span>
                                </div>
                                <div className="flex items-center gap-3 p-3 bg-neutral-900/50 border border-neutral-800 rounded">
                                    <Shield className="text-emerald-500" size={16} />
                                    <span className="text-xs text-neutral-300">데이터 지연 없음</span>
                                </div>
                            </div>
                        </div>

                        <div className="p-4 bg-emerald-900/20 border border-emerald-900/50 rounded">
                            <div className="flex items-start gap-3">
                                <CheckCircle2 className="h-4 w-4 text-emerald-500 mt-0.5" />
                                <div className="space-y-1">
                                    <p className="text-xs font-medium text-emerald-400">모든 기능 활성화됨</p>
                                    <p className="text-[10px] text-emerald-300/70">
                                        토스 앱에서 InsiderPulse를 이용해 주셔서 감사합니다.
                                    </p>
                                </div>
                            </div>
                        </div>
            ...
            {/* Subscription Info - Hide in Apps-in-Toss */}
            {!ENV_CONFIG.isAppintos && (
                <div className="bg-[#0a0a0a] border border-neutral-900 p-6 rounded-sm">
                     <div className="flex items-center gap-3 mb-6 border-b border-neutral-800 pb-4">
            ...
            {/* Coupon / Promotions Section - Hide in Apps-in-Toss */}
            {!ENV_CONFIG.isAppintos && (
             ...
             {/* Payment Info - Hide in Apps-in-Toss */}
             {!ENV_CONFIG.isAppintos && user?.stripeCustomerId && (
                 <div className="bg-[#0a0a0a] border border-neutral-900 p-6 rounded-sm">
