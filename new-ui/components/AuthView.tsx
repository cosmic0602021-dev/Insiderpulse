...
import { ArrowRight, Globe, Lock, Fingerprint, ShieldCheck, Database, Scan, Target, Crosshair, Activity, X } from 'lucide-react';
...
const AuthView: React.FC<AuthViewProps> = ({ lang, onLogin, onClose }) => {
  ...
  useEffect(() => {
    const spawnInterval = setInterval(() => {
        // Don't clutter the screen too much
        if (document.hidden) return;

        const id = Date.now();
        // Random position within 20-80% to keep inside circle somewhat
        const angle = Math.random() * 2 * Math.PI;
        // distribute randomly within radius
        const r = Math.sqrt(Math.random()) * 40; // 0-40% radius
  ...
  return (
    <div className="w-screen h-screen flex bg-[#030303] text-neutral-300 overflow-hidden font-mono selection:bg-emerald-900 selection:text-emerald-50 relative">
      <style>{`
      ...
      {/* Close Button (Only if onClose prop is provided) */}
      {onClose && (
          <button 
            onClick={onClose}
